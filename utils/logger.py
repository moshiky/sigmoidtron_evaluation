...
class Logger(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        if not os.path.exists(os.path.dirname(self.__log_file_name)):
            os.mkdir(os.path.dirname(self.__log_file_name))

        self.__log_lock = threading.Lock()
        self.log('Logger created', should_print=False)

    def error(self, msg, should_print=True):
        self.log('ERROR: {msg}'.format(msg=str(msg)), should_print)
